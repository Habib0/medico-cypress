...
Cypress.Commands.add("fieldValidationTemplateType",()=>{
    ...
    cy.waitForGenericLoader()
    cy.xpath(templateObject.clickTemplateTypesTab).click({force:true})
    cy.xpath(templateObject.clickNewTemplateTypeBtn).should('be.visible').click({force:true})
...
Cypress.Commands.add("saveTemplateType",()=>{
    // cy.task("getCache", "randomNumber").then((a: any) => {
        cy.wait(2000)
        cy.get(templateObject.titleInput).type('Automated Template Type ' + randomNumber)
        cy.xpath('//button[@class="btn btn-md btn-outline-secondary" and text()=" Save "]').click({force:true})
        cy.saveToast()
...
Cypress.Commands.add('editSaveTemplateType',()=>{
    ...
    cy.waitForGenericLoader()
    cy.xpath(templateObject.clickTemplateTypesTab).click({force:true})
    cy.get(templateObject.gridSearch).clear().type('Automated Template Type')
    cy.waitForGenericLoader()
    cy.wait(3000)
    cy.get(templateObject.btnEdit).eq(0).click({force:true})
    // cy.get(templateObject.addPopupVisible).should('be.visible')
    cy.get(templateObject.titleInput).clear().type('Edit Automated Tempalte Type '+ randomNumber)
    cy.xpath('//button[@class="btn btn-md btn-outline-secondary" and text()=" Save "]').click({force:true})
})
